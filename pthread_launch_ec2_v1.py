...
def do_create_ebs(client, cnt, param, vol_id, p_lock):
...
def create_ebs(client, total_ebs, param, cnt_thread):
    ...
    if remain > 0:
        ...
        th = multiprocessing.Process(target=do_create_ebs,
                    args=(client, remain, param, vol_id, p_lock))
        thread_list.append(th)
    for th in thread_list:
        th.start()
        #time.sleep((cnt_ebs_th+cnt_thread)//20)
    for th in thread_list:
        th.join()
    #pool.close()
    #pool.join()
...
def attach_ebs_ec2(ec2, instance, vol_id, ebs_cnt_ec2, cnt_thread):
    ...
    for th in thread_list:
        th.start()
        #time.sleep((ec2_cnt_th+cnt_thread)//20)
    for th in thread_list:
        th.join()
...
if __name__ == "__main__":
    ...
    if args.i == 'run':
        vol_id = create_ebs(client, total_ebs, param_ebs, pthread_cnt)
        print("created total vol:", len(vol_id))
        inst = create_ec2(client, param_ec2)
        attach_ebs_ec2(ec2, inst, vol_id, ec2_ebs_each, pthread_cnt)
